/**
 * 
 * @module ZDistrictUtil
 * @class
 * @classdesc 全国行政区划省市编码
 */
let district = [{
        "name": "北京市",
        "shortName": "北京市",
...
sortByProvince(district);


/**
 * @exports ZDistrictUtil
 * @classdesc 行政区划处理工具类。此类为Object实例，不需要new
 * @class
 */
let ZDistrictUtil = {
...
/**
 * 获取省记录
 * @param {Object} filter 省份过滤器
 * @return {Object}
 */
ZDistrictUtil.getItem = function (filter) {
    for (let i = 0, len = district.length; i < len; i++) {
        let ok = true;
        for (let n in filter) {
            if (district[i][n] !== filter[n]) {
                ok = false;
                break;
            }
        }
 ...
 * 获取城市记录
 * @param {Object} filter 省份过滤器
 * @param {Object} cityFilter 城市过滤器
 * @return {Object}
 */
ZDistrictUtil.getCityItem = function (filter, cityFilter) {
    let prov = this.getItem(filter);
    let cities = prov.cities;
    for (let j = 0, clen = cities.length; j < clen; j++) {
        let ok = true;
        for (let n in cityFilter) {
            if (cities[j][n] !== cityFilter[n]) {
                ok = false;
                break;
            }
        }
...
/**
 * 获取省会城市列表
 * @param {String} [filter] 要过滤查询的省会名称
 * @return {Array}
 */
ZDistrictUtil.getCapitalCities = function (filter) {
    let r = [];
    for (let i = 0, len = district.length; i < len; i++) {
        let prov = district[i];
        let cities = prov.cities;
        for (let j = 0, clen = cities.length; j < clen; j++) {
            if (filter) {
                if (cities[j].name === filter && cities[j].capital === '1') {
                    r.push(cities[j]);
                }
            } else {
                if (cities[j].capital === '1') {
                    r.push(cities[j]);
                }
            }
        }
...
/**
 * 获取拼音首字母分组
 * @param {Number} type 0省，1市分组排序
 * @return {Object}
 */
ZDistrictUtil.getPinYinFirstLetterGroup = function (type) {
    let r = {};
    if (type) {
        for (let i = 0, len = district.length; i < len; i++) {
            let prov = district[i];
            ...
            for (let j = 0, clen = cities.length; j < clen; j++) {
                ...
                let py = cities[j].pinyin;
                let firstLetter = py.charAt(0);
                let hasData = r[firstLetter];
                if (hasData) {
                    hasData.push(cities[j]);
                } else {
                    r[firstLetter] = [cities[j]];
                }
            }
        }
    } else {
        for (let i = 0, len = district.length; i < len; i++) {
            let py = district[i].pinyin;
            let firstLetter = py.charAt(0);
            let hasData = r[firstLetter];
            if (hasData) {
                hasData.push(district[i]);
            } else {
                r[firstLetter] = [district[i]];
            }
        }
    }
 ...
 * 获取湖南省矩形范围
 */
ZDistrictUtil.getHunanProvinceExtent = function () {
    return [108.78712833400004, 24.642016279000075, 114.2548597550001, 30.12895185900004];
};


export default ZDistrictUtil;